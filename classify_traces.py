...
def main():
    ...
    if nb_training_traces > 500:
        print("You can choose 500 training traces at most")
        sys.exit()
    ...
    num_testing_traces = np.array([i for i in range(500)], dtype=np.int16)
    num_training_traces0 = np.array([i for i in range(500)], dtype=np.int16)
    num_training_traces1 = np.array([i for i in range(500,1000)], dtype=np.int16)
    ...
    for test in tqdm(range(nb_tests)):
        ...
        print("Training step time =", end - start)

        # Test LDA
        print("Testing the LDA classifier for 5 subsets of 100 traces")
        scores = np.zeros(5)
        cms = np.zeros((5,2,2))
        ...
        # Average results for the testing subsets
